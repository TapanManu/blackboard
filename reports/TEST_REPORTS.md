# Proposed write-up: the live five-agent A/B run

Two files need edits to record the 2026-09-07 run, plus one open question about
whether an unreproducible measurement belongs in `MEASURED.md` at all.
...
contradictions, module load-bearing analysis, test coverage, setup-bundle
consistency, deferred-layer triggers). Identical prompts and identical requested
depth in both arms; only the reporting path differed. Arm A returned prose. Arm
B wrote a digest plus body to `bb://sedai/tasks.<lane>/result/exp1` and replied
in three lines.
...
Arm B additionally found three Arm A missed: `.mcp.json.example`'s `uvx --from .`
...
expensive than writing a digest plus a body, and the write cost dominates the
load cost.
...
multi-agent task cheaper or faster overall." That's now too strong. Replacement:

> **What is proven and what is not.** The board demonstrably does what it claims
> at the cost it claims. One live five-agent A/B run has now been recorded —
> 93.6% less parent context, 39% less total agent spend, no quality loss
> ([Measured results](MEASURED.md)) — but that is a single run on this
> repository, not the four-arm benchmark in
...
This one cannot be reproduced — it depended on a live session, a specific model,
and prompts that exist only in my transcript. That's a real break with the file's
standard. Two ways to handle it: label it as I have, or additionally commit the
ten prompts and the raw measurements to `bench/` so someone could repeat the
shape of it.

I'd also fix the three defects the run surfaced in the same commit, since two of
them are in files I shipped today. Say the word and I'll write it up.
