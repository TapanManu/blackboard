# Findings — Answers to Q1–Q15, Requirements, Instructions

> **Scope note (revised):** the answers below describe the full design space. Only **L0 (store) + L1 (protocol)** are being built now; orchestration, governance and cloud are deferred and evidence-gated. See `docs/08-layers.md` and `docs/09-value.md`.
...
Nuance worth keeping: **prompt caching is per-prefix, not per-session.** Two sessions that begin with a byte-identical prefix can hit the *same* cached prefix on the provider side. That is not shared memory — neither can read the other's data — but it does mean a shared, stable, canonical protocol prefix (D13) is cheaper for the *second* agent that uses it. That is a real, exploitable property of the blackboard's skill text.
...
**Answer: store canonical JSON in KV namespaces (B+C), render per shape at read time, keep relations in an edge table (D).** See ADR D3/D6.
...
5. **Runbook reuse** — repeated instructions live on the board (R2), not re-typed and re-tokenized each session.
...
2. **Trust Score (D8).** Planner consumes `accepted ∧ Trust ≥ 0.80`; `0.60–0.80` with caveat; `<0.60` re-derive.
...
1. **Tool-schema surface.** Every tool definition is re-sent every turn. ≤12 tools, ≤1,400 tokens total (D9). Cutting a 6k-token tool surface to 1.4k saves ~4.6k **per turn, per agent** — often the single biggest line item.
2. **URIs instead of payloads.** Pass `bb://…` / `bb-artifact://sha256/…` in arguments; never inline a blob.
3. **Server-side budgets.** `budget_tokens` on every read, with explicit truncation reporting (D9). The server, not the model, enforces the ceiling.
...
Additionally: enable **prompt caching on the protocol/skill prefix** (D13), and keep the MCP server's tool list *static* — a tool list that varies by session defeats prefix caching for every agent.
...
## Q14. Is a queue / message broker required? *(answer applies to L2, which is deferred)*
**No.** A `task` table with atomic claim-by-lease *is* a work queue with the right semantics, and `event(seq)` + long-poll *is* a pub/sub log. SQLite WAL handles the local case; Postgres `LISTEN/NOTIFY` the cluster case. Adding Kafka/RabbitMQ/Redis buys ordering and fan-out guarantees this workload does not need, at the cost of another daemon (violating R4 and R10). Revisit only above ~50 concurrent workers or cross-cluster fan-out — and then add NATS JetStream *in front of* the events table, not instead of it. See D11.
...
|---|---|---|
| R1 | Token/context optimization across agents & sessions | Digest-first reads (D4), budgets (D9), URI passing, artifact offload (D7) |
...
| R3 | A2A benefits — 2+ agents faster & cheaper | Claim/lease parallelism (D5), digest fan-in, no transcript copying (Q4/Q12) |
| R4 | Simple install / setup / rapid use / disconnect | `uvx blackboard-mcp init && serve`; one config block; `--ephemeral` for throwaway workspaces (D14) |
...
| R8 | Strictly local / sandboxed storage | SQLite + local artifacts, loopback/UDS only, no telemetry, no egress (D1). Cloud mode is opt-in and in-cluster. |
...
4. **Benchmark score as PoV output** — `docs/06-benchmarks.md` defines the harness and the report card.
5. **Single Opus vs ecosystem+blackboard** — the paired-run protocol in `docs/06-benchmarks.md`.

### In / out of scope

| In scope — **L0 store** | In scope — **L1 protocol** | Out of scope |
|---|---|---|
| Entries, URIs, versions, append-only history | Skill prompt + naming conventions | How many agents; which model per role |
| Mandatory digests; projections (digest/fields/TSV/full/ref) | Read-escalation ladder | **Task scheduling, claim, leases** *(L2, deferred)* |
| `expect_version` CAS | Resume recipe on a well-known URI | **Trust scoring, contest, curator** *(L3, deferred)* |
| Topic scoping + capability tokens | Reference role prompts | **Postgres, K8s, KEDA** *(L4, deferred)* |
