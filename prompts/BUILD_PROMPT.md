...
You are building **`blackboard-mcp`**: a local, agent-agnostic **context store** (L0) plus its **usage protocol** (L1). Nothing else.

**Read first, they are normative:** `docs/08-layers.md` (scope boundary), `docs/00-decisions.md` (the choices, with layer tags), `docs/02-data-model.md` (schema — build only the `[L0]` tables), `docs/03-api-mcp.md` (five tools), `docs/09-value.md` (what success means and when to stop). Where this prompt and the docs disagree, the docs win — flag the conflict rather than guessing.
...
- the `task` table, `claim_task`, leases, the reaper, `watch_events`, `heartbeat_task` — **L2, deferred**
- trust scoring, `contest_state`, the status lifecycle, the curator, compaction — **L3, deferred**
- the Postgres driver, S3 artifacts, HTTP transport, JWT, Helm, KEDA — **L4, deferred**
- a `hello_state` tool, a `patch_state` tool, a board-health tool — these are **conventions or CLI**, not agent tools

If you find yourself writing `acquire_lock`, a scheduler, or a trust weight, stop and re-read `docs/08-layers.md`. Scope creep back into orchestration is the top risk in the register.
...
**Day 1 —** `uv` project, Python 3.12, `src/blackboard/`. Deps: `mcp`, `aiosqlite`, `pydantic`, `click`. `store/base.py` (the interface), `store/sqlite.py` (the `[L0]` tables and all four PRAGMAs), `uri.py` (parse/render/validate + `topic_glob` matching), `tokens.py` (`est_tokens()`, document its error bound; it must be the single estimator used for both budgets and metrics).
...
**Day 5 — L1, and the test that matters most.** Write `skills/blackboard/SKILL.md` conventions into working form: URI naming, the well-known `bb://<ws>/run/state/current` resume URI, the escalation ladder, digest authoring guidance. Then build the **resume test**: snapshot a mid-run board, start a fresh client, run the documented resume recipe, and assert (a) total tokens < 3,000 and (b) the recovered state contains goal, frontier, and the last ten decisions.

## Week 2 — measure, then stop and report
Implement `bench/` for arms A, B, C on suites 1, 2, 4 (`docs/06-benchmarks.md`). Log every model call to JSONL including `cache_read_input_tokens` and `cache_creation_input_tokens`. `bench/report.py` prints the report card and **refuses to print a savings number when the TSR gate fails, stating why.**

Then apply the kill criteria in `docs/09-value.md` and report the verdict. **Do not begin L2 on your own initiative** — a human decides that from the numbers.
...
- When you hit a design question the docs do not answer, add an ADR to `docs/00-decisions.md` with the alternatives you rejected and its layer tag, then proceed. Only stop to ask if the answer changes the L0 schema.
